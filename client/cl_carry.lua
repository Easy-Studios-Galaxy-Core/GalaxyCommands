...
RegisterCommand("carry",function(source, args)
	...
	if exports["rpemotes"]:IsPlayerCrouched() then 
		return print("Crouutched kein carry erlaubt")
	end
	if exports["rpemotes"]:IsPlayerCrawling() then 
		return print("Crawling kein carry erlaubt")
	end
	if exports["jobs_creator"]:isPlayerHandcuffed() then 
		return print("Tod kein carry erlaubt")
	end
	if exports["rpemotes"]:IsPlayerProne() then 
		return print("Proned kein carry erlaubt")
	end
	if exports["rpemotes"]:IsPlayerInAnim() then 
		return print("Proned kein carry erlaubt")
	end
	if exports.wasabi_crutch:IsWheelchairActive() then 
		return print("Bei rollstuhl kein carry erlaubt")
	end
	if exports.wasabi_crutch:IsCrutchActive() then 
		return print("Bei Krèken kein carry erlaubt")
	end
	if not carry.InProgress then
		...
		if closestPlayer then
			...
			if targetSrc ~= -1 then
				...
				drawNativeNotification("~r~Es ist niemand in deiner Nähe!")
			end
		else
			drawNativeNotification("~r~Es ist niemand in deiner Nähe!")
		end
