...
local closing = false
local dispayWatch = false
local coords, rot

if not Config.Suicide.Enabled then return end

RegisterCommand('suicide', function()
	for _, g in ipairs(Config.Suicide.Weapons) do
...
CreateThread(function()
    while true do
        ...
        coords = GetEntityCoords(ped)
        if suicideanim then
            DrawText3D(coords.x, coords.y, coords.z, '[E] - Selbstmord begehen | [~r~X] - Abbrechen')
        end
